...
UIControl* SettingsPanel::CreateExternalControl(u32 id){
    ...
    this->controlCount++;
    button->Load(SIP::buttonFolder, "Settings", variant, this->activePlayerBitfield, 0, false);
    return button;
...
UIControl *SettingsPanel::CreateControl(u32 id){
    ...
    if (id < this->radioCount){
        ...
        radioButtonControl->Load(this->buttonsPerRow[id], data->GetSettings()->pages[this->sheetIdx].radioSetting[id], SIP::controlFolder, "SettingsRadioBase", variant, "SettingsRadioOption", optionVariants, 1, 0, 0);
        ...
        upDownControl->Load(this->optionsPerScroller[id], data->GetSettings()->pages[this->sheetIdx].scrollSetting[id], SIP::controlFolder, "SettingsUpDownBase", variant, "SettingsUpDownButtonR", "RightButton",
        ...
        valueControl->Load(SIP::controlFolder, "SettingsUpDownValue", "Value", "SettingsUpDownText", "Text");
...
void TextUpDownValueControlwID::SetMessage(u32 optionId){
    ...
    u32 bmgId = BMG_SCROLLER_SETTINGS + (((SIPUI::SettingsPanel*)this->parentGroup->parentPage)->sheetIdx<<12) + (this->id+1<<4)+ optionId;
    text->SetMsgId(bmgId);
}
}//namespace SIPUI
