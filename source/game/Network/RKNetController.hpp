...
public:
    ...
    u8 unknown_0xec[4];
    SplitRACEPointers *splitToSendRACEPackets[2][12]; //split pointers for the outgoing packets, double buffered, indexed by aid
    SplitRACEPointers *splitReceivedRACEPackets[2][12]; //split pointers for the incoming packets, double buffered, indexed by aid
    PacketHolder *fullSendPackets[12]; //combined outgoing packets, indexed by aid
    u64 lastRACEToSendTimes[12]; //time when last sent to that aid
    u64 lastRACERecivedTimes[12]; //time when last received from that aid
    u64 RACEToSendTimesTaken[12]; //last send time minus the time of the send before it
    u64 RACERecivedTiemsTaken[12]; //last receive time minus the time of the receive before it
    u8 lastRACESendAid; //aid a packet was last sent to
    u8 fullRecivedPackets[12][0x2e0]; //1 per aid
    u8 padding[3];
    StatusData localStatusData; //8 bytes, see http://wiki.tockdom.com/wiki/MKWii_Network_Protocol/Server/gpcm.gs.nintendowifi.net#locstring
    ...
    u8 unknown_0x2757;
    bool hasprofanity; //1 if name is bad
    u8 padding2[3];
    s32 badWordsNum; //number of bad strings in the profanity check
    u8 unknown_0x2760[3];
    s32 vr;
    s32 br;
    s32 lastSendBufferUsed[12]; //last full send buffer used for each aid, 0 or 1
    s32 lastRecivedBufferUsed[12][8]; //last Recived buffer used for each packet per aid, 1 or0
    s32 currentSub; //index of the current sub to use, 0 or 1
    u8 aidsBelongingToPlayerIds[12]; //index is player id, value is aid
    u32 disconnectedAids; //1 if disconnected, index 1 << aid
    u32 disconnectedPlayerIds; //index 1 << playerId
